      ...
      try {
        ...
        if (analysis) {
          console.log("Processing analysis object:", {
            hasAnalysis: !!analysis,
            analysisShape: analysis ? Object.keys(analysis) : [],
          });

          // First validate and transform analysis data to match schema
          const transformedAnalysis = {
            style: {
              current: analysis.style?.toString() || "",
              influences: Array.isArray(analysis.influences) ? analysis.influences : [],
              similarArtists: Array.isArray(analysis.similarArtists) ? analysis.similarArtists : [],
              period: analysis.period,
              movement: analysis.movement
            },
            composition: {
              structure: analysis.composition?.toString() || "",
              balance: analysis.balance?.toString() || "",
              colorTheory: analysis.colorTheory?.toString() || "",
              focusPoints: Array.isArray(analysis.focusPoints) ? analysis.focusPoints : [],
              dynamicElements: Array.isArray(analysis.dynamicElements) ? analysis.dynamicElements : []
            },
            technique: {
              medium: analysis.technique?.toString() || "",
              execution: analysis.execution?.toString() || "",
              skillLevel: analysis.skillLevel?.toString() || "",
              uniqueApproaches: Array.isArray(analysis.uniqueApproaches) ? analysis.uniqueApproaches : [],
              materialUsage: analysis.materialUsage?.toString() || ""
            },
            strengths: Array.isArray(analysis.strengths) ? analysis.strengths : [],
            improvements: Array.isArray(analysis.improvements) ? analysis.improvements : [],
            detailedFeedback: analysis.detailedFeedback?.toString() || "",
          ...
          console.log(
            "Transformed analysis object for insertion:",
            JSON.stringify(transformedAnalysis, null, 2)
          );

          try {
            // Insert feedback with properly transformed analysis object
            ...
            console.log("Feedback stored successfully:", {
              ...
              analysisKeys: feedbackEntry.analysis ? Object.keys(feedbackEntry.analysis) : []
            });
            ...
            console.error("Error inserting feedback:", {
              ...
              analysisShape: Object.keys(transformedAnalysis),
              rawAnalysis: analysis,
              transformedAnalysis: transformedAnalysis
            });
            // Even if feedback insertion fails, return the artwork
            res.json({
              ...artwork,
              feedback: [],
              analysisError: error instanceof Error ? error.message : "Error storing feedback",
            });
          }
        } else {
          ...
          feedback: sql<string>`
            COALESCE(
              jsonb_agg(
                jsonb_build_object(
                  'id', ${feedback.id},
                  'analysis', ${feedback.analysis}::jsonb
                )
              ) FILTER (WHERE ${feedback.id} IS NOT NULL),
              '[]'::jsonb
            )`,
          styleComparisons: sql<string>`
            jsonb_build_object(
              'asCurrent', COALESCE(
                (
                  SELECT jsonb_agg(
                    jsonb_build_object(
                      'id', sc1.id,
                      'comparison', sc1.comparison::jsonb
                    )
                  )
              ...
              'asPrevious', COALESCE(
                (
                  SELECT jsonb_agg(
                    jsonb_build_object(
                      'id', sc2.id,
                      'comparison', sc2.comparison::jsonb
                    )
                  )
